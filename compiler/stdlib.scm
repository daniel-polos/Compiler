  ...
  (let ((null? null?) (car car) (cdr cdr))
		(letrec ((fl-loop
					(lambda (proc obj lst)
							(if(null? lst)
								obj
								(fl-loop proc (proc obj (car lst)) (cdr lst))))))
				(lambda (p o l)
					(fl-loop p o l)))))
	...
	(let ((null? null?) (car car) (cdr cdr) (cons cons))
		(letrec ((reverse-loop
					(lambda (lst acc)
						(if(null? lst)
							acc
							(reverse-loop (cdr lst) (cons (car lst) acc))))))
			(lambda (lst)
				(reverse-loop lst '())) )))

(define cons*
  (let ((null? null?) (car car) (cdr cdr) (cons cons) (fold-right fold-right) (reverse reverse))
		(lambda args
			(if(null? args)
				args
				(fold-right cons (car (reverse args)) (reverse (cdr (reverse args))) )))))
